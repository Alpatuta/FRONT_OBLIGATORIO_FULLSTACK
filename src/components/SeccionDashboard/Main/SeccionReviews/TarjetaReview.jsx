...
const Stars = ({ value }) => (
  <span style={{ color: "#f59e0b", letterSpacing: "2px", fontSize: "15px" }}>
    {"★".repeat(value)}
    <span style={{ color: "var(--border-md)" }}>{"★".repeat(5 - value)}</span>
  </span>
);

const TarjetaReview = ({ review, onEdit, onDelete }) => {
  const [confirmDelete, setConfirmDelete] = useState(false);

  const recetaNombre =
    typeof review.receta === "object" && review.receta !== null
      ? review.receta.titulo
      : "Receta";
